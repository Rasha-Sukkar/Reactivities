...
{
    public static async Task SeedData(AppDbContext context, UserManager<User> userManager) //making request for db async to await response from db ince it will take time
    {

        var users = new List<User>
            {
                new () {DisplayName= "Bob", UserName="bob@test.com", Email="bob@test.com", EmailConfirmed=true},
                new () {DisplayName= "Tom", UserName="tom@test.com", Email="tom@test.com", EmailConfirmed=true},
                new () {DisplayName= "Jane", UserName="jane@test.com", Email="jane@test.com", EmailConfirmed=true}
            };
        ...
        if (context.Activities.Any()) return; //if there re any activtities return
        ...
        context.Activities.AddRange(activities); //telling to Add a list of activities into the database context.
        await context.SaveChangesAsync(); //this will add and save
    }
}